...
forked from https://github.com/xream/scripts/tree/main/surge/modules/sub-store-scripts/mega*/
...
const resolver = getVal('resolver') || 'ali'
    ...
    if (!isIPV4(ip)) {
      ...
      if (mock) {
        ...
        try {
          if (resolver === 'google') {
          ...
          } else if(resolver === 'ali') {
            const res = await $.http.get({
              url: `https://223.5.5.5/resolve?name=${encodeURIComponent(p.server)}&type=A`,
              headers: {
                'User-Agent':
                  'Mozilla/5.0 (Macintosh; Intel Mac OS X 12_3_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.60 Safari/537.36',
              },
            ...
            console.log(answers)
            ip = answers[answers.length - 1].data
            console.log('↓ ip')
            console.log(ip)
            if (!isIPV4(ip)) {
              throw new Error(`${resolver} ${p.server} 解析 ${ip} 不是 IPV4`)
